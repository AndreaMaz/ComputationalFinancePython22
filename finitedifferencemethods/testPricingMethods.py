...
def plotCallWithExactSolution():
    ...
    for maturity in np.arange(0.1, tmax+0.1, 0.1):
        ...
        plt.plot(x, exactSolution, 'r')
        plt.plot(x, solutionExplicitEuler, 'bo-')
        plt.plot(x, solutionImplicitEuler, 'go-')
        plt.plot(x, solutionCrankNicolson, 'mo-')
...
def plotSolutionBarrierOption():
    ...
    for maturity in np.arange(0.1, tmax+0.02, 0.02):
        #we would have problems getting the Black-Scholes formula for x=0, since
        #it would divide by zero                                                     
