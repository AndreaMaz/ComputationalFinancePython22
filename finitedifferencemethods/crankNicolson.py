...
class CrankNicolson(PricingWithPDEs):
    ...
    def initializeTerms(self):
        ...
        self.multiplyTermSecondDerivative = self.dt/(self.dx*self.dx)
        ...
        diagonal = 1 + 0.5 * self.multiplyTermSecondDerivative * self.x[1:-1]**2 * self.sigma(self.x[1:-1])**2 \
            + 0.5 * self.dt * self.r
        lowerDiagonal = - 0.5 * self.multiplyTermSecondDerivative *  self.x[2:-1]**2 * self.sigma(self.x[2:-1])**2 \
            + self.r * self.multiplyTermFirstDerivative * self.x[2:-1]
        upperDiagonal = - 0.5 * self.multiplyTermSecondDerivative *  self.x[1:-2]**2 * self.sigma(self.x[1:-2])**2 \
    ...
    def getSolutionAtNextTime(self):
        ...
        secondDerivatives = self.multiplyTermSecondDerivative * (uPast[2:] - 2*uPast[1:-1] + uPast[:-2])
           
        addingTerm = 0.5 * secondDerivatives * self.x[1:-1]**2 * self.sigma(self.x[1:-1])**2 \
        ...
        knownTerm[0] += 0.5 * uPast[0] \
           * (0.5 * self.multiplyTermSecondDerivative * self.x[1]**2 * self.sigma(self.x[1])**2 \
              -  self.multiplyTermFirstDerivative * self.r * self.x[1])
           
        #we multiply by 0.5 is because the scheme is given by
        #(u_i^{n+1}-u_i^n)/dt = (F_i^{n+1}-F_i^n)/2
        knownTerm[-1] += 0.5 * uPast[-1] \
           * (0.5 * self.multiplyTermSecondDerivative * self.x[-2]**2 * self.sigma(self.x[-2])**2 \
