...
class CliquetOptionForCVWithArrays:
    """
    This class is designed to compute the payoff of a Cliquet option for a general
    matrix representing the returns of a process over sub-intervals. It operates with arrays.
    ...
    # to call this method only by typing the name of the class: that is, one has to write
    # _CliquetOptionForCV__setNonTruncatedPayoffs(returnsForAllSimulations) to call this
    # method from outside the class
    def __setNonTruncatedPayoffs(self, returnsForAllSimulations):

        # we set the attribute of the class
        self.nonTruncatedPayoffs = [self.nonTruncatedPayoffSingleTrajectory(rowOfMatrixOfReturns) for rowOfMatrixOfReturns in returnsForAllSimulations]
    ...
    def getPayoffs(self, returnsForAllSimulations, globalFloor=- np.inf,
        ...
        Parameters
        ----------
        returnsForAllSimulations : array
        ...
        if (globalFloor is -np.inf) and (globalCap is np.inf):
            ...
            payoffs = np.minimum(np.maximum(self.nonTruncatedPayoffs, globalFloor), globalCap)

        return payoffs

    def discountedPriceOfTheOption(self, returnsForAllSimulations, interestRate,
                                   globalFloor=- np.inf, globalCap=np.inf):
        """
        It returns the discounted price of the Cliquet option, as the discounted
        average of the payoffs for a single simulation of the returns.
